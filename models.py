...
def step(model, optimizer, batch, alpha, r2, learnable_r = False):
    ...
    if learnable_r:
        ...
        loss += loss_penalty
        # loss_l2 = model.MIU * torch.norm(model.fc1.weight) # for input space
        loss_l2 = model.MIU * torch.norm(model.fc2.weight) # for net3_learnable
        loss += loss_l2
...
def eval_step(model, x, y, alpha, r2, learnable_r = False):
    ...
    with torch.no_grad():
        ...
        if learnable_r:
            ...
            loss_l2 = model.MIU * torch.norm(model.fc2.weight)
            loss += loss_l2
...
def eval_combined(model, dl_combined, alpha, r2, learnable_r = False):
    ...
    # z = z.detach().numpy()
    # z = z>0.
    # z = np.sum(z, axis=1) # count of non-zero logits
    # return z, confidence.numpy()
...
def mlp_step(model, optimizer, batch):
    model.train()
    optimizer.zero_grad()
...
def mlp_eval_combined(model, dl_combined):
    model.eval()
    for i, batch in enumerate(dl_combined):
        x = batch[0]
        with torch.no_grad():
...
class MLP2(nn.Module):
    def __init__(self, inputs, features, bias=False):
        super().__init__()
        self.inputs = inputs
        self.fc1 = nn.Linear(inputs, features, bias= True)
        # without bias, converge to a bad acc for Moons dataset.
